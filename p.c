...
// 		step = TILE;
// 		offset = 1;
// 		// hit_y = data->player_y + (((int)data->player_y  % TILE));
...
// 		map_y = (hit_y + offset) / TILE;
// 		map_x = hit_x / TILE;
// 		if (hit_x < TILE || hit_x > ((data->width * TILE)  - TILE))
// 			return(2147483647);
...
// 			// printf("we have a hit: y : %lf x : %lf char : %c\n", hit_y , hit_x , data->map[(int)(hit_y / TILE)][(int)hit_x /TILE]);
// 			// printf("hit\n");
// 			if (!CUBE3D)
// 			{
// 				*horizontal_x = hit_x;
...
// 		}
// 		hit_y += step;
// 		if (hit_y < TILE || hit_y > ((data->height * TILE) - TILE))
// 			return(2147483647);
// 	}
// 	return (-1);
// }
...
// 		if (hit_y < TILE || hit_y > ((data->height * TILE)))
// 			return(2147483647);
// 		map_y = hit_y / TILE; 
// 		map_x = (hit_x + offset) / TILE;
// 		if (data->map[map_y][map_x] == '1')
// 		{
// 			// printf("hit_x: %lf hit_y: %lf map_y: %d map_x : %d step : %d\n", hit_x, hit_y, ((int)hit_y / TILE), ((int)hit_x / TILE) + (step / TILE), step);
...
// 		}
// 		hit_x += step;
// 		if (hit_x < 0 || hit_x > (data->width * TILE))
// 			return(2147483647);
// 	}
// 	return (2147483647);
// }
...
// 			// printf("next_y: %f %d next_x: %f %d\tmap: %c\n", next_y,(int)next_y / TILE,  next_x, (int)next_x /TILE, data->map[(int)next_y / TILE][(int)next_x / TILE]);
// 			if ( data->map[(int)((next_y)  / TILE)][(int)(next_x / TILE)] == '0' || data->map[(int)next_y / TILE][(int)next_x  / TILE] == 'P')
// 			{
...
// 	if (!CUBE3D)
// 	{
// 		mlx_data.mlx = mlx_init(data.width * TILE, data.height * TILE, "Minimap", true);
